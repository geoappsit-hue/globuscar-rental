...
export function buildTemplateVars(data: ContractData) {
  ...
  return {
    ...
    INSURANCE_TYPE: { kasko: 'КАСКО', super_kasko: 'СуперКАСКО', full_coverage: 'Полное покрытие' }[data.rental.insuranceType] || data.rental.insuranceType,
    INSURANCE_DAILY_RATE: data.rental.insuranceDailyRate.toString(),
    INSURANCE_TOTAL: data.rental.insuranceTotal.toString(),
    // Legacy aliases kept for backward compat with existing templates
    SUPER_KASKO: { kasko: 'КАСКО', super_kasko: 'СуперКАСКО', full_coverage: 'Полное покрытие' }[data.rental.insuranceType] || '',
    SUPER_KASKO_TOTAL: data.rental.insuranceTotal.toString(),
